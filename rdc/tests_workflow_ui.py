...
class RDCWorkflowUITests(TestCase):
    def setUp(self):
        self.user = User.objects.create_user(
            username="supervisor_workflow_ui",
            password="123",
            is_staff=True,
        )
        ...
        self.projeto = Projeto.objects.create(
            codigo="PRJWUI",
            nome="Projeto Workflow UI",
            cliente="Cliente Workflow UI",
        )

        self.disciplina = Disciplina.objects.create(
            codigo="DISWUI",
            nome="Disciplina Workflow UI",
        )

        self.area = AreaLocal.objects.create(
            projeto=self.projeto,
            codigo="AREAWUI",
            descricao="?rea Workflow UI",
        ...
        self.client.login(username="supervisor_workflow_ui", password="123")

    def test_detalhe_exibe_painel_workflow(self):
